    ...
    <>
      <header className="bg-white/80 backdrop-blur-lg border-b border-slate-200 sticky top-0 z-10">
        <div className="px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-3xl font-bold text-slate-900">Domain Blacklist</h2>
            <p className="text-slate-600 mt-1">Manage {blacklist.length} blacklisted domains</p>
          </div>
          <button
            onClick={() => setShowAddModal(true)}
            className="flex items-center gap-2 px-6 py-3 bg-linear-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:shadow-lg hover:shadow-blue-500/30 transition-all duration-200 font-medium"
          >
      ...
      <div className="p-8 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {status === 'succeeded' && blacklist.map((item: any) => (
          <div key={item._id} className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => handleRemove(item.domain)} className="text-slate-400 hover:text-red-600 transition-colors">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
              <h3 className="text-lg font-semibold text-slate-900 truncate" title={item.domain}>{item.domain}</h3>
              <p className="text-slate-600 text-sm mt-1 mb-3">{item.reason || 'No reason provided'}</p>
            </div>
